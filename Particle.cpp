...
Particle::Particle(RenderTarget& target, int numPoints, Vector2i mouseClickPosition) : m_A(2, numPoints)
{
	...
	//Set plane center and size
	m_cartesianPlane.setCenter(0, 0);
	m_cartesianPlane.setSize(target.getSize().x, (-1.0) * target.getSize().y);
	//Set Particle center on plane
	m_centerCoordinate = target.mapPixelToCoords(mouseClickPosition, m_cartesianPlane);
	//Random pixel velocities
	m_vx = (((float)rand() / (RAND_MAX)) * 321);
	if (rand() % 2 != 0)
		m_vx = m_vx * -1;
	m_vy = (((float)rand() / (RAND_MAX)) * 321);
	//Assign colors
...
void Particle::draw(RenderTarget& target, RenderStates states) const
{
	...
	{
		Vector2f pos(target.mapCoordsToPixel(Vector2f(m_A(0, j - 1), m_A(1, j - 1)), m_cartesianPlane));
